...
$steps->Given( '/^an active theme with the given templates:$/',
	function( $world, $stream ) {
		$dest = $world->variables['RUN_DIR'] . '/wp-content/themes/test-theme';
		$world->proc( "rm -rf $dest" )->run_check();
		@mkdir( $dest, 0777, true );
		$world->copy_dir( __DIR__ . '/../extra/test-theme', $dest );

		$files = array_map( 'trim', explode( PHP_EOL, (string) $stream ) );
		foreach ( $files as $file ) {
			@mkdir( dirname( $dest . '/' . $file ), null, true );
			file_put_contents( $dest . '/' . $file, "<!-- $file -->" );
		}

		$world->proc( 'wp theme activate test-theme' )->run_check();
	}
);

$steps->Given( '/^the given named templates:$/',
	function( $world, $stream ) {
		$dest = $world->variables['RUN_DIR'] . '/wp-content/themes/test-theme';

		$files = array_map( 'trim', explode( PHP_EOL, (string) $stream ) );
		foreach ( $files as $file ) {
			$template_name = ucwords( preg_replace( '/[-_]+/', ' ', basename( $file, '.php' ) ) );
			$contents      = <<<EOT
<?php
/**
...
EOT;
			...
			file_put_contents( $dest . '/' . $file, $contents );
		}
...
$steps->Given( '/^a child theme with the given files:$/',
	function( $world, $stream ) {
		$dest = $world->variables['RUN_DIR'] . '/wp-content/themes/test-child-theme';

		$world->proc( 'wp scaffold child-theme test-child-theme --parent_theme=test-theme --activate' )->run_check();
